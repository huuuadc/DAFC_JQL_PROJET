		...
		<div class="ur-form-row">
			<div class="ur-form-grid">
					...
					if ( 'yes' === $lost_password_enabled ) {
						?>
								<p class="user-registration-LostPassword lost_password">
									<a href="/sign-in/forgot-password"><?php echo esc_html( $labels['lost_your_password'] ); ?></a>
								</p>
							<?php
					}
					?>
					...
					<p class="form-row">
						...
						<div>
							<?php if($enable_ajax) { ?>
							<input type="submit" class="user-registration-Button button ur-submit-button secondary" id="user_registration_ajax_login_submit" name="login" value="<?php echo esc_html( $labels['login'] ); ?>" />
							<span></span>
							<?php } else { ?>
							<input type="submit" class="user-registration-Button button secondary" name="login" value="<?php echo esc_html( $labels['login'] ); ?>" />
							<?php } ?>
						</div>
						<?php if(is_checkout()) : ?>
							<input type="hidden" name="redirect" value="<?php echo wc_get_checkout_url(); ?>"/>
						<?php else : ?>
				...
				if ( $users_can_register ) {
					...
					if ( ! empty( $url_options ) ) {
						...
						if ( ! empty( $label ) ) {
							?>
                                <span><?php echo get_option( 'user_registration_general_setting_registration_label' ); ?></span>
								<a class="button secondary is-outline" href="<?php echo get_option( 'user_registration_general_setting_registration_url_options' ); ?>"> <?php _e( 'Create Account', 'glamoutlet' ); ?>
									</a>
								<?php
						} else {
