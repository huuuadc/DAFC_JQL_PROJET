 ...
 * Shortcode handler.
 */
class Shortcode extends \A_Z_Listing\Singleton
{
    /**
    ...
    function handle($attributes)
    {
        ...
        $attributes = shortcode_atts(array('alphabet' => '', 'display' => 'posts', 'exclude-posts' => '', 'exclude-terms' => '', 'get-all-children' => 'false', 'group-numbers' => '', 'grouping' => '', 'hide-empty-terms' => 'false', 'numbers' => 'hide', 'parent-post' => '', 'parent-term' => '', 'post-type' => 'page', 'return' => 'listing', 'target' => '', 'taxonomy' => '', 'terms' => ''), $attributes, 'a-z-listing');
        ...
        $grouping_obj = new \A_Z_Listing\Grouping($grouping);
        $numbers_obj = new \A_Z_Listing\Numbers($attributes['numbers'], $group_numbers);
        if ('terms' === $attributes['display'] && !empty($attributes['taxonomy'])) {
            ...
            $query = array('taxonomy' => $taxonomies, 'hide_empty' => $hide_empty);
            $terms_string = '';
            ...
            if (!empty($terms_string)) {
                ...
                $terms = \array_unique(\array_filter(\array_map('intval', \array_map('trim', $terms)), function ($value) {
                    return 0 < $value;
                }));
                $query = wp_parse_args($query, array($terms_process => $terms));
            }
            if (!empty($attributes['parent-term'])) {
                ...
                if (0 < $parent_id) {
                    $query = wp_parse_args($query, array($parent_selector => $parent_id));
                }
            }
            $a_z_query = new \A_Z_Listing\Query($query, 'terms');
        } else {
            ...
            if (!empty($attributes['exclude-posts'])) {
                ...
                $exclude_posts = \array_unique(\array_filter(\array_map('intval', \array_map('trim', $exclude_posts)), function ($value) {
                    return 0 < $value;
                }));
            ...
            if (!empty($attributes['exclude-terms'])) {
                ...
                $tax_query[] = array('taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => $ex_terms, 'operator' => 'NOT IN');
            }
            if (!empty($tax_query)) {
                $query['tax_query'] = $tax_query;
            }
            $a_z_query = new \A_Z_Listing\Query($query, 'posts');
        }
        $target = '';
