...
class Query
{
     ...
     */
    public function __construct($query = null, $type = 'posts', $use_cache = \true)
    {
        global $post;
        $this->get_alphabet();
        ...
        if ('terms' === $type) {
            ...
            if (\is_array($query)) {
                ...
                $query = wp_parse_args(array('taxonomy' => (array) $taxonomies), $defaults);
            }
            /**
            ...
            if (!$query instanceof \WP_Query) {
                ...
                $query = wp_parse_args($query, array('post_type' => 'page', 'numberposts' => -1, 'nopaging' => \true));
            }
    ...
    protected static function get_section($page = 0)
    {
        ...
        if (null !== $section_name && !\in_array($section_name, $sections, \true)) {
            $section_name = null;
            $section_object = null;
        }
        if (AZLISTINGLOG) {
            ...
            foreach ($items as $item) {
                ...
                foreach ($item_indices as $index => $index_entries) {
                    if (\count($index_entries) > 0) {
                        if (\in_array($index, \array_keys($this->alphabet), \true)) {
            ...
            foreach ($this->alphabet_chars as $character) {
                if (!empty($indexed_items[$character])) {
                    \usort($indexed_items[$character], function ($a, $b) {
                         ...
                         */
                        $sort = apply_filters('a_z_listing_item_sorting_comparator', $default_sort, $atitle, $btitle);
                        if (\is_int($sort)) {
                            if (AZLISTINGLOG) {
    ...
    {
        echo $this->get_the_letters($target, $style);
    ...
    public function the_listing()
    {
        ...
        $templates = array('a-z-listing-' . $section . '.php', 'a-z-listing.php');
        if ($post) {
            \array_unshift($templates, 'a-z-listing-' . $post->post_name . '.php');
        }
    ...
    public function get_the_item_type()
    {
        ...
        if (\is_string($current_item)) {
            $item = \explode(':', $current_item, 2);
            if (isset($item[0]) && \in_array($item[0], array('post', 'term'), \true)) {
                return $item[0];
            }
        }
        if (\in_array($this->type, array('terms', 'posts'), \true)) {
