							...
							$registration_form
								...
								.on("submit", function (event) {
									...
									var data = {
										...
										resend_smsotp: jQuery('input[name="resend_smsotp"]').length > 0 ? jQuery('input[name="resend_smsotp"]').val() : '', //namdeveloper
										value_smsotp: jQuery('input[name="sms_otp"]').length > 0 ? jQuery('input[name="sms_otp"]').val() : '', //namdeveloper
									};
									...
									$this
										.find(".ur-submit-button")
										.html('<div class="lds-ring"><div></div><div></div><div></div><div></div></div> Processing');
									...
									$.ajax({
										...
										complete: function (ajax_response) {
											...
											if (!ajaxFlag["status"]) {
												...
												if(typeof response.data.code != 'undefined' && response.data.code.indexOf('sms_otp_') != -1) {
													jQuery('.user-registration form.register').addClass('opt-verify');
													jQuery('.user-registration form.register #smsOTPSection').html(''+
														'<div data-field-id="sms_otp" class="ur-field-item field-number sms-otp">'+
															'<div class="form-row" id="sms_otp_field" data-priority="">'+
																'<label for="sms_otp" class="ur-label">Enter Verification Code</label>'+
																'<span class="input-wrapper"> '+
																	'<input data-rules="" data-id="sms_otp" type="number" class="input-text without_icon input-number ur-frontend-field " name="sms_otp" id="sms_otp" step="1">'+
																'</span>'+
															'</div>'+
															'<div class="form-row otp-status">'+
																'<a id="resendOTP" onclick="stopOTPTimer=true; resendOTP(\'form.register\')" >Resend OTP</a>'+
																'<span id="otpTimeRemain"></span>'+
															'</div>'+
															'<input type="text" name="resend_smsotp" value="false" style="display:none"/>'+
														'</div>');
													//jQuery('.sms-otp').show();
													if(response.data.code == 'sms_otp_sent'){
														type = "message";
														success_message_position = "1";
														stopOTPTimer = false;
														otpTimer(120);
													}
													jQuery('.ur-submit-button').html('Verify OTP');
												}
												//namdeveloper
												if(typeof response.success != 'undefined' && response.success == true) {
													jQuery('.user-registration form.register').removeClass('opt-verify');
													jQuery('.user-registration form.register #smsOTPSection').html('');
													//window.scrollTo(0, 0);
													jQuery('#triggerRegisterSuccessPopup').trigger('click');
												}
												form.show_message(
													message,
													type,
